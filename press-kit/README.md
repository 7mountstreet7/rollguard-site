...
- `panels-v5.zip` — App Store screenshot set: 7 panels (1284×2778) plus the App Preview poster frame (886×1920) — ✓ in repo. **The poster frame inside this zip (`02-film-poster-start-mock.png`) is the START-SCAN screen and carries no scan figures — correct as shipped, and NOT to be re-rendered.** It was briefly confused with the index page's `/images/film-poster.jpg`, a different picture that did carry figures; **that file was deleted on 2026-09-11 and the `<video>` now shows the film's own first frame instead**, so this is the only "film poster" in the repo. Supersedes `panels-v4.zip`, which was retired and deleted 2026-09-10 because its closer panel still read "iOS 17+".
